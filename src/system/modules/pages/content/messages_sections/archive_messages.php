...
$varArchivedMessages = Messages::fetchUsersArchivedMessages($varUsersID);

// "Header Option" Element Settings.

$hdOption->setLevel(2);
$hdOption->setContent(Locales::getTitle("archive"));

// "Paragraph Notice" Element Settings.

$parNotice->setID("communication-notice");
$parNotice->setAlignment(FParagraph::ALN_CENTER);
$parNotice->setContent(Locales::getParagraph("empty-archive"));
...
if ($varArchivedMessages != null)
{
    for ($i = 0; $i < count($varArchivedMessages); $i ++)
    {
        if ($varArchivedMessages[$i]["read_status"] == 0)
            $varArchivedMessages[$i]["read_status"] = Locales::getCore("unread");
        else
            $varArchivedMessages[$i]["read_status"] = Locales::getCore("read");

        $tempRow = new FTableRow();

        $tempRow->addCell(new FTableCell(null, null, $i + 1));
        $tempRow->addCell(new FTableCell(null, null, $varArchivedMessages[$i]["date_sent"]));
        $tempRow->addCell(new FTableCell(null, null, new FAnchor(null, null, $varLinkPreifx . Locales::getLink("view-archived-message") . "&" . Locales::getVariable("id") . "=" . $varArchivedMessages[$i]["id"], null, $varArchivedMessages[$i]["title"])));
        $tempRow->addCell(new FTableCell(null, null, $varArchivedMessages[$i]["read_status"]));
